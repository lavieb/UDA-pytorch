...
def get_uda_train_test_loaders(train_set,
    ...
    train1_sup_ds = TransformedDataset(train1_set,
                                       transform_fn=lambda dp: (train_transform_fn(dp[0]), dp[1]))
    test_ds = TransformedDataset(test_set, transform_fn=lambda dp: (test_transform_fn(dp[0]), dp[1]))

    original_transform = lambda dp: train_transform_fn(dp[0])
    augmentation_transform = lambda dp: unsup_transform_fn(dp[0])
...
# def get_uda2_train_test_loaders(dataset_name, path, num_labelled_samples, batch_size, num_workers, unlabelled_batch_size=None, pin_memory=True):
#
#     if "cifar" in dataset_name.lower():
#         train_ds, test_ds, num_classes = get_cifar_train_test_datasets(dataset_name, path)
#     else:
...
#     train1_unsup_loader = DataLoader(train1_unsup_ds, batch_size=unlabelled_batch_size, num_workers=num_workers, pin_memory=pin_memory)
#     train2_unsup_loader = DataLoader(train2_unsup_ds, batch_size=unlabelled_batch_size, num_workers=num_workers, pin_memory=pin_memory)
#
#     test_loader = DataLoader(test_ds, batch_size=batch_size * 2, num_workers=num_workers, pin_memory=pin_memory)
#
#     return train1_sup_loader, train1_unsup_loader, train2_unsup_loader, test_loader
